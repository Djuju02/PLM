...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_global_ingredient'])) {
  ...
  $g_price = $_POST['default_unit_price'];
  $g_tva = $_POST['default_tva'];

  $stmt_g = $mysqli->prepare("INSERT INTO ingredients_global (name, reference, default_unit_price, default_tva) VALUES (?,?,?,?)");
...
<div class="container">
  ...
  <h1>Gérer les Sous-Produits (BOM)</h1>
  <p>Voici la liste globale des ingrédients disponibles. Vous pouvez en ajouter, modifier ou supprimer.</p>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="actions">
          <a href="edit_ingredient_global.php?id=<?php echo $g['id']; ?>" class="btn-edit">Modifier</a>
          <a href="delete_ingredient_global.php?id=<?php echo $g['id']; ?>" class="btn-delete" onclick="return confirm('Supprimer cet ingrédient ?')">Supprimer</a>
        </td>
      </tr>
      <?php endwhile; ?>
    </tbody>
  ...
  <form method="post" style="max-width:300px;">
    ...
    <div class="form-group">
      ...
      <input type="number" step="0.01" name="default_unit_price" value="5.00" required>
    </div>
    <div class="form-group">
      <label>TVA (%)</label>
      <input type="number" step="0.01" name="default_tva" value="20.00" required>
    </div>
